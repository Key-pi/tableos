...
| Admin operational actions | Admin invokes safe service transitions rather than raw edits | B2 locked order/bill/session actions and service-backed BillingRequest actions; lifecycle/derived/relation fields are readonly | PARTIAL_CURRENT | Financial allocation/ledger Admin controls remain B3; PostgreSQL concurrency execution remains environment-gated |
